...
void Swt::inversegradient()
{
	for(int i = 1; i< height - 1;i++)
	{
		for(int j = 1; j< width - 1;j++)
		{
			max_x.at<float>(i,j) = 0 -  max_x.at<float>(i,j); 
			max_y.at<float>(i,j) = 0 - max_y.at<float>(i,j);
		}
	}
}
...
void Swt::Edges(vector< vector<cv::Point2i> >& cc)
{
	...
	//cv::imshow(" test",StrokeImage);

	for(int i = 1;i < height;i++)
		for(int j = 1; j< width;j++)
		if (StrokeImage.at<float>(i, j) != StrokeImage.at<float>(i, j - 1) || 
			StrokeImage.at<float>(i, j) != StrokeImage.at<float>(i - 1, j))
				this->EdgeImage.at<unsigned char>(i,j) = 255;

	/*for(int j = 1; j< width;j++)
		for(int i = 1;i < height;i++)
			if(StrokeImage.at<float>(i,j)!=StrokeImage.at<float>(i - 1,j))
				this->EdgeImage.at<unsigned char>(i,j) = 255;*/
					...
					if((EdgeImage.at<unsigned char>(nowi,nowj) == 255)&&(nowi != i)&&(nowj != j)){
						...
						if((max_x.at<float>(i,j)*max_x.at<float>(nowi,nowj) + max_y.at<float>(i,j)*max_y.at<float>(nowi,nowj)) < - 0.5)
						{
							while(qb->length)
							{
								...
								if (SPIvaule == 0 || SPIvaule > step_lengh){
									StrokeImage.at<float>(pt.i,pt.j) = step_lengh;
								}
								/*else if(SPIvaule > step_lengh){
									StrokeImage.at<float>(pt.i,pt.j) = step_lengh;
									}*/
							}
						}
